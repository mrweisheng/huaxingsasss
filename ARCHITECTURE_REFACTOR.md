...
> 状态：已归档（2026-06-08 已废弃 interrupt 机制，改为计划驱动安全门 + 纯聊天确认）
...
| 场景 | 预期行为 |
|------|---------|
| 上传 PDF 合同（有文本） | AI 展示分析结果，用户自然语言确认后创建合同 |
| 上传 PDF 合同（扫描件，无文本） | VL 分析 → AI 展示 → 用户确认 → 创建合同 |
| 上传 Word 合同 | AI 展示 → 用户确认 → 创建合同 |
| 上传 Excel 合同 | AI 展示 → 用户确认 → 创建合同 |
| 上传图片合同 | VL 分析 → AI 展示 → 用户确认 → 创建合同 |
