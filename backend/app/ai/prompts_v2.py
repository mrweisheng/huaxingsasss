...
- 不同币种金额分行展示，不要折算后相加
- 合同总费用可能同时标注人民币和港币两个金额（如"￥195000 / $ 226673"），此时以付款计划中各期的货币为准确定主货币。分析结果的 `payment_terms` 中每期都有独立的 `currency` 字段，创建付款记录时应使用各期自带的币种
...
- `create_contract` 返回的 `auto_payments` 才是系统实际创建的**付款记录**——只有当 VL 分析结果中该期 `is_paid=true` 时才会创建
- `auto_payments` 为空 = 只创建了合同和付款计划，没有任何实际付款记录
- 向用户汇报时，用"付款计划"而非"付款记录"——"合同已录入，付款计划为：① 定金 ¥50,000 ② ..."，不要说"已生成待确认付款记录"
...

{
  ...
  "payment_terms": [{"name": "款项说明（含业务主体+性质）", "amount": 金额, "currency": "本期币种（CNY/HKD），根据该期付款描述中的货币符号（¥/HK$/$）或文字（人民币/港币/港幣）判断", "due_date": "YYYY-MM-DD", "condition": "支付条件", "is_paid": "布尔值。判断该期款项是否已实际支付/收取——需综合合同上下文语义判断（如'已付''已缴纳''已到账''付清''甲方已收'等表述），不能仅靠关键词匹配。合同只写了付款条件和时间但没有明确表示已支付时，is_paid应为false"}],
...
4. is_paid 由你根据合同上下文语义判断该期是否已实际支付/收取，而非简单匹配关键词。合同只写了付款时间/条件（如"签约即日支付""完成后三日内支付"）但没明确表示已到账/已收取的，is_paid 应为 false。只有合同明确表达了款项已实际完成支付/收取的含义时才为 true
5. 严禁推断信息，合同没写的不能编造
