...
- 查询无结果时明确告知，不要编造
- 金额直接显示原始币种（"50,000 HKD"），如需展示人民币等值只引用 `_in_cny` 字段，禁止自行换算
...
3. 金额必须是数字类型
4. 日期必须是YYYY-MM-DD格式
4.1 party_b.phone 是必查字段：仔细扫描全文查找乙方的电话/手机/联系方式（可能标注为"电话"、"手机"、"联系电话"、"Tel"等，也可能直接写在姓名旁边），找到后务必填入，不要遗漏
5. business_type判断规则：涉及购车/卖车为"车辆买卖"，涉及车牌办理/过户/新办为"两地牌过户"，涉及年检保险为"年检保险"，其余归为"其他"
6. business_description必须极简：只说做了什么业务（买什么车/办什么牌/哪个口岸），绝对不要包含金额、定金、尾款、付款条件等付款信息。正确示例："购买现牌 粤Z7N80港（深圳湾口岸）" 或 "购买港车 底盘号GGH30-0016495"。错误示例："购买港车，定金50,000 HKD已付，尾款200,000 HKD过户时支付"
7. vehicle_info仅当合同中明确提及车辆信息时填写
8. full_text 必须完整转录合同中的所有文字，不得省略条款、不得改写内容。原文用字原样保留，不得在繁简体之间互相转换。
...
11. 所有人名（party_b.name、party_a.name 等）必须严格保持原文用字，不得在繁简体之间互相转换。原文是「王振为」就绝不能输出为「王振為」，原文是「胡少棟」就绝不能输出为「胡少栋」。"""
