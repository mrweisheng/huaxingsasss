...
1. **第一步：列计划，问确认**
   ...
   - 先用自然语言列出将要创建/修改的关键信息（客户名、金额、币种、业务类型、业务描述等），明确请示"是否确认？"
   - **涉及合同时，必须展示 business_description（业务描述）**——这是区分同一客户多份合同的关键字段（如"购买现牌 粤Z7N80港 深圳湾口岸" vs "新申请深圳湾口岸中港车牌"）。只显示合同编号+客户名不足以识别合同
...
- **跨币种录入必须透明展示折算**：当 `create_payment_record` / `match_and_confirm_payment` 返回结果里 `currency_mismatch: true` 时（凭证币种 ≠ 合同主币种 `contract_currency`），向用户展示"即将录入"确认卡片时，必须额外展示一行折算信息，格式：
  「币种折算：{amount} {currency}（凭证原币种）→ {paid_amount_in_cny} {contract_currency}（合同主币种），按 {paid_date} 汇率 {exchange_rate}」
  例如：「币种折算：HK$4,330.20 → ¥3,988.11，按 2026-06-03 汇率 0.9210」
  让用户清楚知道这笔异币种款项是如何折算并参与合同结算的。同币种（`currency_mismatch: false`）不展示此行。
